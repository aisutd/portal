...
export function AdminApplicationsManager({ initialApplicationId, embedded = false }: { initialApplicationId?: string; embedded?: boolean }) {
  ...
  useEffect(() => { const review = selectedSubmission?.reviews[0]; setScore(review?.score ?? null); setNotes(review?.notesInternal ?? ""); }, [selectedSubmissionId, selectedSubmission]);

  async function updateSubmission(status?: Status) {
    if (!detail || !selectedSubmission) return;
    setSaving(true); setError(null);
    try {
      const response = await fetch(`/api/admin/applications/${detail.id}/submissions/${selectedSubmission.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ score, notes, ...(status ? { status } : {}) }) });
  ...
  function selectNextApplicant() {
    ...
    setSelectedSubmissionId(submissions[(index + 1 + submissions.length) % submissions.length].id);
  }
