  ...
  return (
    <div className="relative">
      <button
        ...
        aria-expanded={open}
        className={`flex size-[28px] cursor-pointer items-center justify-center rounded-full text-[16px] leading-none transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-soft ${
          open
      ...
      {open && (
        ...
        >
          <div className="sticky top-0 z-10 shrink-0 border-b border-border-soft bg-row-soft px-[14px] py-[10px]">
            <p className="truncate font-body text-[13.5px] font-bold text-ink">{memberName}</p>
            ...
            <div className="mt-[5px] flex flex-wrap items-center gap-[4px]">
              {currentBadges.length > 0 ? (
                currentBadges.map((badge) => (
                  <span
                    key={badge.label}
                    className="rounded-full px-[7px] py-[2px] font-mono text-[9.5px]"
                    style={{
                ...
                <span className="font-mono text-[9.5px] text-ink-faint">Member · no programs</span>
              )}
            </div>
          </div>
          ...
          {confirmRemove ? (
            <div className="flex flex-col gap-[10px] p-[14px]">
              <p className="font-body text-[12.5px] leading-[17px] text-ink">
                Remove <span className="font-bold">{memberName}</span> from the portal?
              </p>
              <p className="font-body text-[11.5px] leading-[16px] text-ink-faint">
              ...
              {error && (
                <p
                  role="alert"
                  className="rounded-[8px] bg-danger-ink/10 px-[10px] py-[7px] font-body text-[11.5px] leading-[16px] text-danger-ink"
                >
                  {error}
                </p>
              )}
            ...
            <div className="flex flex-col gap-[14px] p-[14px]">
              <div>
                <p className="font-techno text-[9.5px] uppercase tracking-[1.2px] text-ink-faint">
                  Role
                </p>
                  ...
                  {ASSIGNABLE_USER_ROLES.map((option) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex cursor-pointer items-center justify-between rounded-[9px] border px-[11px] py-[7px] text-left font-body text-[13.5px] transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-soft ${
                          selected
                  ...
                  {ASSIGNABLE_PROGRAMS.map((program) => {
                    ...
                    return (
                      <button
                        ...
                        aria-pressed={selected}
                        className={`cursor-pointer rounded-full border px-[11px] py-[5px] font-body text-[12px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-soft ${
                          selected
                            ? "border-transparent"
              ...
              {saved && (
                <p
                  role="status"
                  className="rounded-[8px] bg-[#eaf4e8] px-[10px] py-[7px] font-body text-[12px] font-semibold text-[#356b2e]"
                >
              ...
              <div className="flex items-center justify-between gap-[8px]">
                <button
                  ...
                  className="cursor-pointer rounded-[6px] px-[2px] font-body text-[12px] font-semibold text-danger-ink underline-offset-2 transition-colors hover:underline focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-danger-ink/25"
                >
                  Remove
                </button>
