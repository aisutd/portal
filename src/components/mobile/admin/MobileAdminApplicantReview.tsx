    ...
    <MobileScreen withBottomNavPadding={false}>
      ...
      <div className="flex flex-col gap-[10px]">
        <input
          ...
          className="h-[40px] w-full rounded-[8px] bg-search-field px-[12px] font-mono  text-search-ink placeholder:text-search-ink focus:outline-none"
        />
      ...
      <div className="flex flex-col gap-[16px] rounded-[16px] border border-border-soft bg-white p-[18px]">
        <div className="flex items-center gap-[12px]">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="font-mobile-display  font-bold text-ink">
              {applicantDetail.name}
            </h3>
            <p className="font-mono  leading-[15px] text-ink-faint">
        ...
        <p className="font-mono  text-ink-faint">{applicantDetail.appliedAt}</p>
        ...
        <div className="flex flex-col gap-[10px]">
          {applicantDetail.questions.map((q) => (
            <div key={q.prompt} className="flex flex-col gap-[6px]">
              <h4 className="font-mobile-display  font-bold text-ink">
                {q.prompt}
              </h4>
              {q.lines.map((w, i) => (
        ...
        <div className="flex flex-col gap-[12px] rounded-[14px] bg-row-soft p-[16px]">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono  text-ink-faint">your scores</span>
          </div>
          {applicantDetail.rubric.map((c) => (
            <div key={c.label} className="flex items-center justify-between gap-[8px]">
              <span className="font-mobile-body  text-ink-muted">{c.label}</span>
              <ScoreScale selected={c.selected} />
            </div>
          ))}
          <div className="h-px w-full bg-border-soft" />
          <div className="flex items-center justify-between">
            <span className="font-mobile-body  font-bold text-ink">Your overall</span>
        ...
        <div className="flex flex-col gap-[10px]">
          ...
          <div className="flex items-center justify-between rounded-[16px] border border-brand bg-brand-soft px-[16px] py-[14px]">
            <span className="font-mobile-body  font-bold text-brand-dark">
              Combined avg
            </span>
            <span className="font-mobile-display  font-bold text-brand-dark">
