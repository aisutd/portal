...
export async function MobileEventDetail({ eventId }: MobileEventDetailProps) {
  ...
  return (
    <MobileScreen>
      <Link 
        href="/events" 
        className="inline-flex items-center py-1 font-mono  font-bold text-brand transition-opacity hover:opacity-80"
      >
        ← All Events
      </Link>

      <div className="flex h-55 w-full shrink-0 items-center justify-center rounded-2xl bg-photo shadow-sm overflow-hidden">
        <span className="font-mono  font-medium tracking-[2px] text-photo-text">
          PHOTO
        </span>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <h1 className="font-mobile-display  font-bold leading-tight text-ink">
            {event.title}
          </h1>
          <p className="font-mono  font-medium text-ink-muted">
        ...
        <p className="font-mobile-body  leading-relaxed text-ink-muted">
          {event.description}
        </p>
      </div>
      ...
      <div className={`mt-2 flex flex-col items-center justify-center gap-[16px] rounded-[20px] p-[24px] shadow-sm ${cardStyle}`}>
        {isPast ? (
          <>
            <h2 className={`font-mobile-display  font-bold ${
              attended ? "" : isRsvpd ? "text-red-700" : "text-ink"
            }`}>
              {attended ? "Attended" : isRsvpd ? "Missed Event" : "Event Passed"}
            </h2>

            <p className="text-center font-mobile-body  text-ink-muted">
              {attended
